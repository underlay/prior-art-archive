...
/*--------*/
export const GET_LOGIN_LOAD = 'login/GET_LOGIN_LOAD';
...
export function getLogin() {
	return (dispatch) => {
		...
		.then((result) => {
			dispatch({ type: GET_LOGIN_SUCCESS, result });
		})
		.catch((error) => {
			dispatch({ type: GET_LOGIN_FAIL, error });
		});
	};
}

export function postLogin(slug, password) {
	return (dispatch) => {
